# ancoraDocs KnowledgeBase v2.26
...
- Admin users can enable a PDF split fallback for PDFs that repeatedly fail Google File Search ingestion. The app imports smaller page-range PDFs into File Search while keeping the original PDF as the local citation source.
...
- The PDF split fallback is a workaround for Google ingestion failures. It does not create a local vector database or custom RAG system; Google File Search still imports, chunks, embeds, indexes, retrieves, and cites the page-range PDF parts. Citation page links are mapped back to the original locally archived PDF when Google returns the app metadata.
